...
import notification.SendMailUtil;
...
public class TimesheetSubWebService {
    ...
    public String timeSheetSubmit(String startTime, String endTime, String task, String project ,int totalTime) throws Exception{
        ...
        Query qManager = em.createNamedQuery("Employee.findByRole", Employee.class);
        qManager.setParameter("role", "manager");
        List<Employee> manager = qManager.getResultList();
            ...
            if (task.equals(invoiceList.get(i).getTask()) && project.equals(invoiceList.get(i).getProject()) && tongThoiGian <= invoiceList.get(i).getInvoiceTime()) {
                ...
                try { 
                    ...
                    if((invoiceList.get(i).getStartDate().equals(dateStart) || invoiceList.get(i).getStartDate().before(dateStart)) && (invoiceList.get(i).getEndDate().equals(dateEnd) || invoiceList.get(i).getEndDate().after(dateEnd))){
                        ...
                        SendMailUtil.sendMail(workerEmail, "Bảng chấm công của bạn đã không hợp lệ\n"
                                + "Task: " + task +"\nProject: " + project +
                                "\nThời gian bắt đầu: " + dateStart + "\nThời gian kết thúc: " + dateEnd);
                        
                        SendMailUtil.sendMail(manager.get(0).getEmail(), "Bảng chấm công của nhân viên " + workerName + " đã không hợp lệ\n"
                                + "Task: " + task +"\nProject: " + project +
                                "\nThời gian bắt đầu: " + dateStart + "\nThời gian kết thúc: " + dateEnd);
                        return "date wrong";
                    }
